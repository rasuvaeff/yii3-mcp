...
1. **`ToolCallInterceptorInterface`** *(done, unreleased)* — a public extension
   point wrapping every `tools/call`
   (`intercept(ToolCallContext $context, callable $next)`),
...
2. **AI audit trail** — an interceptor recording every `tools/call` (client
   ...
   the enterprise answer to "what did the AI do in our system". Ships as a
   separate bridge package (`rasuvaeff/yii3-mcp-audit-log-bridge`) so the
   core stays dependency-free.
3. **Session budget** *(done, unreleased)* — a per-session `tools/call`
   counter with a configurable limit (MCP error once exhausted). Protection
