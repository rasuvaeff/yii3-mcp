...
аргументов (не-скаляр, разделитель пути, отсутствующий path-параметр,
некорректный `dryRun`) бросают `InvalidToolArgumentException` и сформулированы
так же. До вызывающего доходят только эти два типа - обычный
`InvalidArgumentException` из PSR-17/PSR-18 снизу (неразбираемый URI запроса,
имя delegated-заголовка, не совместимое с RFC 7230) описывает деплой, а не
вызов, и остаётся за generic internal error от SDK. Единственное место, где
по-прежнему сообщается
