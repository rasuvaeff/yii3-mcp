...
| `openapi-bridge.php` | Allow-listed OpenAPI operations bridged as MCP tools — `tool_names` rename, `OperationModifierInterface`, `readOnlyHint`/tags in `_meta`, `dryRunOperations` preview; the call becomes a real HTTP request (stubbed PSR-18 client) | no |
